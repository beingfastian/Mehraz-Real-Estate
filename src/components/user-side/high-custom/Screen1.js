...
import { buildingicon, buyMaterialLightIcon, high_CustomIcon } from "@/assets";
import Image from "next/image";
import { UserHeader } from "@/components";

// ✅ Import BGs
import materialsBg from "@/assets/images/buy-materials.png";
import highCustomBg from "@/assets/images/high-custom.png";
import propertyBg from "@/assets/images/buy-property.png";
...
const Screen1 = ({ setStep, heading, subheading }) => {
  ...
  const backgroundImage = useMemo(() => {
    if (pathname === "/buy-materials") return materialsBg.src;
    if (pathname === "/high-custom") return highCustomBg.src;
    if (pathname === "/buy-property") return propertyBg.src;
    return localBackgroundImage.src; // fallback
  }, [pathname]);
  ...
  return (
    <div className="flex flex-grow h-full absolute top-0 left-0 w-full">
      ...
      <motion.div
        ...
        className="relative z-[1] min-h-full w-full flex items-center justify-center bg-no-repeat bg-center bg-cover before:absolute before:z-[-1] before:top-0 before:left-0 before:right-0 before:bottom-0 before:bg-gradient-to-b before:from-[#000000e6] before:to-[#3c3c3cb3] flex-grow h-full">
